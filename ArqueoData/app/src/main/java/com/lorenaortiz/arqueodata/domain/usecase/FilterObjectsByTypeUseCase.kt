...
class FilterObjectsByTypeUseCase @Inject constructor(
    private val repository: ArchaeologicalObjectRepository
) {
    operator fun invoke(type: String): Flow<List<ArchaeologicalObject>> {
        return repository.filterObjectsByType(type)
    }
} 